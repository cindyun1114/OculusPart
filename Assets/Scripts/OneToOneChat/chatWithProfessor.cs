...
public class chatWithProfessor : MonoBehaviour
{
    ...
    void setEmbeddPrompt(int length)
    {
        ...
        @"
        這是上傳內容的目錄。
        請完全遵守Instruction進行 Phrase2，依序每個目錄最多問兩個問題。問題也不要重複。
        ...
        目前目錄問題都完成了就進下一個目錄
        當進行到最後的章節""結束""就請在JSON的 reply 中只回 ""太棒了，你準備好了! 面對後方的左邊有個儲思盆，點選它到下個階段""
        注意是要進行到最後的章節""結束後""才在reply只回  ""太棒了，你準備好了! 面對後方的左邊有個儲思盆，點選它到下個階段"" 不要有其他內容 不然扁你
        ";
    }
